  ...
  actions: {
    ...
    Login({ commit }, data) {
      return new Promise((resolve, reject) => {
        ...
        commit('SET_HRM', {})
        // 清空旧的路由数据
        commit('SET_ROUTERS', { addRouter: [], router: {} })

        loginAPI(data)
          .then(res => {
            const resData = res?.data || {}
            const token = resData.token || res?.data || res
            if (!token) {
              return reject({ ...res, msg: '未获取到登录Token' })
            }
            commit('SET_AUTH', { token })
            // 保存登录用户信息到本地存储，供 GetUserInfo 使用
            if (resData.userId) {
              Lockr.set('LOGIN_USER_INFO', resData)
            }
    ...
    GetUserInfo({
      ...
      return new Promise((resolve, reject) => {
        // 优先使用登录时保存的用户信息
        const loginUserInfo = Lockr.get('LOGIN_USER_INFO')
        if (loginUserInfo && loginUserInfo.userId) {
          const userInfo = {
            userId: loginUserInfo.userId || loginUserInfo.employeeId,
            username: loginUserInfo.username || loginUserInfo.mobile,
            realname: loginUserInfo.realname,
            mobile: loginUserInfo.mobile,
            email: loginUserInfo.email || '',
            sex: loginUserInfo.sex || 1,
            deptId: loginUserInfo.deptId,
            deptName: loginUserInfo.deptName || '',
            post: loginUserInfo.post || '',
            parentId: 0,
            status: 1,
            img: ''
          }
    ...
    LogOut() {
      return new Promise((resolve, reject) => {
        const doLogout = () => {
          ...
          Lockr.rm('LOGIN_USER_INFO') // 清除登录用户信息
          // 重置登录状态
          window.__requireLogin = true
          window.__localAuth = false
