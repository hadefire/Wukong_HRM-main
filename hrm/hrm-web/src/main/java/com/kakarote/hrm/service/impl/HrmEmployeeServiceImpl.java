...
public class HrmEmployeeServiceImpl extends BaseServiceImpl<HrmEmployeeMapper, HrmEmployee> implements IHrmEmployeeService {
    ...
    public List<SimpleHrmEmployeeVO> queryAllEmployeeList(String employeeName) {
        LambdaQueryWrapper<HrmEmployee> wrapper = new QueryWrapper<HrmEmployee>().lambda()
                .select(HrmEmployee::getEmployeeId, HrmEmployee::getEmployeeName, HrmEmployee::getPost,
                        HrmEmployee::getEntryStatus, HrmEmployee::getIsDel, HrmEmployee::getDeptId,
                        HrmEmployee::getJobNumber, HrmEmployee::getMobile, HrmEmployee::getEmail,
                        HrmEmployee::getSex, HrmEmployee::getEntryTime)
                .eq(HrmEmployee::getIsDel, 0)
                .like(StrUtil.isNotEmpty(employeeName), HrmEmployee::getEmployeeName, employeeName);
        List<HrmEmployee> hrmEmployeeList = this.list(wrapper);
        List<SimpleHrmEmployeeVO> simpleHrmEmployeeVOList = new ArrayList<>();
        for (HrmEmployee employee : hrmEmployeeList) {
            simpleHrmEmployeeVOList.add(transferSimpleEmp(employee));
    ...
    public SimpleHrmEmployeeVO transferSimpleEmp(HrmEmployee employee) {
        ...
        // 新增字段
        simpleHrmEmployeeVO.setJobNumber(employee.getJobNumber());
        simpleHrmEmployeeVO.setMobile(employee.getMobile());
        simpleHrmEmployeeVO.setEmail(employee.getEmail());
        simpleHrmEmployeeVO.setSex(employee.getSex());
        simpleHrmEmployeeVO.setEntryTime(employee.getEntryTime());
...
//        createTree(deptId, treeDept);
    ...
    public List<OperationLog> addEmployeeField(AddEmployeeFieldManageBO addEmployeeFieldManageBO) {
        ...
        transferEmployee(employee);
        // 设置默认密码（如果未设置）
        if (cn.hutool.core.util.StrUtil.isBlank(employee.getPassword())) {
            employee.setPassword(com.kakarote.hrm.utils.PasswordUtil.getEncodedDefaultPassword());
        }
        save(employee);
        OperationLog operationLog = new OperationLog();
        operationLog.setOperationObject(employee.getEmployeeId(), employee.getEmployeeName());
